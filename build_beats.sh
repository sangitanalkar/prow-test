...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Beats/7.6.2/build_beats.sh
...
#PATCH_URL
PATCH_URL="https://raw.githubusercontent.com/sangitanalkar/prow-test/master/patch"
...
function configureAndInstall() {
	...
	curl -SLO https://dl.google.com/go/go1.13.8.linux-s390x.tar.gz
	...
	sudo python -m pip install appdirs pyparsing packaging setuptools wheel PyYAML termcolor ordereddict nose-timer MarkupSafe virtualenv
	
	# The upgrade of six may fail in certain distros, discard upgrade in such cases
	sudo python -m pip install --upgrade six || true
...
function fileChanges(){
	...
	wget $PATCH_URL/patch.diff -O - | git apply
...
"rhel-8.0" | "rhel-8.1")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing dependencies... it may take some time.\n"
	sudo yum install -y git curl make wget tar gcc libpcap libpcap-devel openssl openssl-devel which acl zlib-devel patch  systemd-devel python2 |& tee -a "${LOG_FILE}"
